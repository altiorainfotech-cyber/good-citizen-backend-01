...
export class CommonService {
  ...
  constructor(
    ...
    (this.TEMP_JWT_ACCESS_SECRET = this.configService.get<string>(
      'TEMP_JWT_ACCESS_SECRET',
    )!),
      (this.TEMP_JWT_ACCESS_EXPIRY = this.configService.get<string>(
        'TEMP_JWT_ACCESS_EXPIRY',
      )!),
  ...
  async decodeToken(access_token: string) {
    try {
      ...
      const decode = await this.jwtService.verifyAsync(access_token, {
        secret: this.JWT_ACCESS_SECRET,
      });
